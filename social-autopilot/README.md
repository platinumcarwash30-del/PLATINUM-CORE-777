...
- Runs daily at 06:00 Europe/Belgrade time.
...
The separate analytics monitor reads Search Console and Google Analytics 4 through their read-only APIs. Scheduled reports run daily at 06:00 Europe/Belgrade time. It does not type searches into Google or scrape result pages. Configure the local values from `.env.analytics-monitor.example`, including this Windows key path outside the repository:

`E:\PLATINUM_CORE_777_WORK\secrets\pc777-search-monitor.json`

After building, run one report with `npm run analytics-monitor:once`, or run the scheduled monitor with `npm run analytics-monitor:dev`. The schedule is 06:00 in `Europe/Belgrade`. If SMTP is configured, the report is sent to `platinum303030@gmail.com`; otherwise it is printed locally.
...
- `.github/workflows/social-autopilot.yml` runs one cycle daily at 06:00 Europe/Belgrade time.
...
- Routine runs are scheduled for 06:00 Europe/Belgrade time. Failed bot workflows trigger an immediate urgent email. SMTP notification requires the existing mail secrets.
...
The notification recipient is fixed by configuration as `platinum303030@gmail.com`. Routine bot reports are sent at 06:00 Belgrade time; operational workflow failures trigger an immediate urgent alert.
...
The production scheduler runs daily at 06:00 in `Europe/Belgrade`. A second check cannot republish a successfully recorded page/platform combination.
